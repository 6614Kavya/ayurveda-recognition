"""
VedaVision — Shared classifier definitions
=============================================
HierarchicalSpeciesClassifier and make_stage1_pipeline live HERE ONLY.

Why this file exists (don't skip this if you're editing the classifier):
joblib/pickle saves a POINTER to a class's module+name, not the class code
itself. If the same class were defined separately in model_training.py and
evaluate.py (or defined in a script run directly, where Python records its
module as "__main__"), a model trained in one script fails to load in the
other with:

    AttributeError: Can't get attribute 'HierarchicalSpeciesClassifier'
    on <module '...'>

By defining the class in exactly one importable module and having both
model_training.py and evaluate.py `from models.species_id.classifier import
...`, the module path recorded at save time always matches the module path
used at load time, regardless of which script trained the model or how it
was invoked.
"""
