...
Run:
    python -m preprocessing.species_id.batch_processor --data dataset/raw --out processed --mode train
    python -m preprocessing.species_id.batch_processor --data dataset/raw --out processed --mode test
...
── CHECKPOINTING REMOVED (this session) ──────────────────────────────────────
Checkpoint CSVs and resume-from-checkpoint were removed. Only the final
full feature CSV (vedavision_features_<mode>.csv) and the classifier CSV
(vedavision_features_<mode>_clf.csv) are written, at the end of the run.
"""
...
def run_batch(data_root: Path,
    ...
    summary = {
        ...
        "n_success"          : n_success,
        "n_fail"             : n_fail,
        "success_rate"       : round(n_success / max(len(tasks), 1) * 100, 2),
